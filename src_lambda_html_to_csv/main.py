...
def get_target_folder_key(source_file_key):
    sub_folder_path = pathlib.Path(source_file_key).parent.relative_to(S3_SOURCE_FOLDER)
    source_file_stem = pathlib.Path(source_file_key).stem
    target_folder_key = pathlib.Path(S3_TARGET_FOLDER).joinpath(sub_folder_path).joinpath(source_file_stem).as_posix()
    return target_folder_key
...
def lambda_handler(event, context):
    ...
    if event.get('Records', None):
        ...
        bucket = event['bucket']
        source_file_key = event['html_file_key']
        pdf_file_key = event['pdf_file_key']
    
    # Result files will be placed at S3_TARGET_FOLDER/soure_file_stem/ folder
    target_folder_key = get_target_folder_key(source_file_key)
    
    local_file_paths = process_file(bucket, source_file_key)
    csv_file_keys = upload_files_to_s3(local_file_paths, bucket, target_folder_key)

    return {
        "bucket": bucket,
        "pdf_file_key": pdf_file_key,
        "html_file_key": source_file_key,
